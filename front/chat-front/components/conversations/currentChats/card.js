...
const Card = ({ conversation, message, owner, friendChat }) => {
  ...
  return (
    <div className="h-24 w-full p-3 border-b-2 border-b-gray-700  flex items-center gap-4 cursor-pointer hover:bg-blue-900" onClick={handleClick}>
      ...
      <div>
        ...
        <p className='text-gray-300 font-light text-1xl '>{owner ? `you:` : ""}  {message.length > 90 ? `${message.substring(0, 95)}... ` : message} </p>
      </div>
    </div>
  );
}
